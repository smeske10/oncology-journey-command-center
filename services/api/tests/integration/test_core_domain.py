...
def database_url() -> str:
    """Return the owner-credential target used for fixture setup and rollback."""
    setup_database_url = settings.require_migration_database_url()
    if not _database_is_reachable(setup_database_url):
        pytest.skip(
            "PostgreSQL MIGRATION_DATABASE_URL is not reachable for core-domain integration test"
        )
