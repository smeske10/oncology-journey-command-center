...
### Controller lint fix: stable case-loading transition

The controller's final lint run identified a production React lifecycle issue: the case-fetch effect synchronously called `setLoadingCase(true)` (and cleared case/error state), which violates `react-hooks/set-state-in-effect`; it also depended only on a selected-patient expression while the selected item object changed independently.
